...
import { extractJobFromText, runPipeline, runDiscovery, makeCvForJob, generateDocument, slug, jobDir, JOBS_DIR } from "../pipeline";
import { getApplications, updateApplicationStatus, saveAcceptedJob, getRecentJobs, getShortlist, getCompaniesCountPerAts } from "../shared/db";
import { readFileSync, existsSync, readdirSync } from "fs";
import { join } from "path";
import type { JobRecord, ResumePayload, FilterResult } from "../shared/types";
...
function startBot() {
  ...
  bot.command("run", async (ctx: Context) => {
    await ctx.reply("🔍 Fetching jobs from all companies → filtering...");
    try {
      const results = await runPipeline();
      if (results.length === 0) {
        await ctx.reply("No matching jobs found this run.");
        return;
      }
  ...
  bot.command("discover", async (ctx: Context) => {
    await ctx.reply("🔍 Running company discovery...");
    try {
      const count = await runDiscovery();
      await ctx.reply(`✅ Discovery complete. ${count} new companies found.`);
    } catch (err: any) {
      await ctx.reply(`❌ Error: ${err.message}`);
    }
  });

  bot.command("shortlist", async (ctx: Context) => {
    const items = getShortlist();
    if (items.length === 0) {
      await ctx.reply("No shortlist. Run /run first.");
      return;
    }
    const lines = items.map((i, idx) =>
      `${idx + 1}. ${fmtScore(i.score)} ${i.company} — ${i.title} (${i.score})\n   ✓ ${i.mustHaveHits.slice(0, 3).join(", ")}`,
    );
    await ctx.reply([`📋 Shortlist (${items.length} jobs):\n`, ...lines].join("\n"));
  });

  bot.command("companies", async (ctx: Context) => {
    const counts = getCompaniesCountPerAts();
    const lines = counts.map((c) => `• ${c.ats}: ${c.count} companies`);
    await ctx.reply([`🏢 Companies per ATS:\n`, ...lines].join("\n"));
  });
  ...
  bot.command("generate", async (ctx: Context) => {
    const match = ctx.message?.text?.match(/\/generate\s+(.+)/);
    if (!match) {
      await ctx.reply("Usage: /generate {jobId}");
      return;
    }
    const jobId = match[1].trim();
    const app = getApplications().find((a) => a.job_id === jobId);
    if (!app) {
      await ctx.reply(`Application ${jobId} not found.`);
      return;
    }
    await ctx.reply(`⏳ Regenerating package for ${app.company}...`);
    try {
      const jdPath = join(JOBS_DIR, app.job_id, "jd.md");
      let description = "";
      if (existsSync(jdPath)) {
        description = readFileSync(jdPath, "utf-8");
      }
      const job: JobRecord = {
        id: app.job_id,
        site: "regenerate",
      ...
      const resume = await doMakeCv(ctx, job, { verdict: "accept", score: 100, reasons: [], must_have_hits: [], missing: [] });
      if (resume) {
        await ctx.reply(`✅ Package regenerated for ${app.company}`);
      }
    } catch (err: any) {
      await ctx.reply(`❌ Error: ${err.message}`);
    }
  ...
  bot.command("status", async (ctx: Context) => {
    ...
    const statusEmoji: Record<string, string> = {
      approved: "✅", ready: "📄", applied: "📬",
      interviewing: "🎙️", offer: "🎉", rejected: "❌",
      ghosted: "👻", withdrawn: "🚫",
    };
    const lines = apps.map((a) =>
      `${statusEmoji[a.status] || "📄"} ${a.company} — ${a.title} | ${a.job_id} | ${a.status}`,
    );
  ...
  bot.command("update", async (ctx: Context) => {
    ...
    if (!match) {
      await ctx.reply("Usage: /update {jobId} {status}. Statuses: approved, ready, applied, interviewing, offer, rejected, ghosted, withdrawn");
      return;
    }
    const jobId = match[1].trim();
    const status = match[2].trim().toLowerCase();
    const validStatuses = ["approved", "ready", "applied", "interviewing", "offer", "rejected", "ghosted", "withdrawn"];
  ...
  bot.command("help", async (ctx: Context) => {
    await ctx.reply(
      [
        "🤖 CV Autopilot v2 Commands:",
        "",
        "/run — Fetch jobs → AI filter → shortlist",
        "/discover — Discover new companies only",
        "/shortlist — Show current shortlist",
        "/describe 1 3 — Show job descriptions",
        "/make 1 3 — Generate CVs for approved jobs",
        "/generate {jobId} — Regenerate documents",
        "/status — View all applications",
        "/update {jobId} {status} — Update status",
        "/companies — Show company count per ATS",
        "/get {jobId} — View application package",
        "",
        "📝 Paste any job description — AI extracts and saves it",
      ].join("\n"),
    );
  });
...
async function doMakeCv(ctx: Context, job: JobRecord, filter: FilterResult): Promise<ResumePayload | null> {
  try {
    const resume = await makeCvForJob(job, filter.score);
