...
import { parseProfile, loadAnswersMarkdown, type ApplicantProfile } from "./profile";
import { applicationRuns, applicationRunSteps } from "../db";
import type { ApplicationRunsRepository } from "../db/repositories/applicationRuns";
import type { ApplicationRunStepsRepository } from "../db/repositories/applicationRunSteps";
...
export async function runApply(options: ApplyRunOptions): Promise<ApplyRunResult> {
  const { runId, jobId, url, profilePath, runsRepo, stepsRepo } = options;
  ...
  runsRepo.updateStatus(runId, "running", { currentUrl: url, outputDir });
  ...
  async function answerFreeTextQuestions(p: Page, jobContext: string) {
    ...
    try {
      const client = new OpenCodeClient({
        ...
        timeoutMs: process.env.OPENCODE_TIMEOUT_MS ? Number(process.env.OPENCODE_TIMEOUT_MS) : 180000,
      });
  ...
  async function pauseForHumanVerification(pageBlocker: string): Promise<ApplyRunResult> {
    ...
    if (keepBrowserOnBlock) {
      ...
      runsRepo.updateStatus(runId, resultStatus, { currentUrl: page.url(), summary: JSON.stringify(buildSummary(0)) });
  ...
  async function resumeAfterHumanVerification(): Promise<ApplyRunResult> {
    ...
    if (pageBlocker) {
      ...
      runsRepo.updateStatus(runId, "needs_user", { currentUrl: page.url(), summary: JSON.stringify(buildSummary(0)) });
  ...
  async function cancelPausedRun(): Promise<void> {
    ...
    runsRepo.updateStatus(runId, "failed", { error: message, currentUrl: page.url(), summary: JSON.stringify(buildSummary(0)) });
    await browser.close().catch(() => {});
  }
  ...
  async function continueApplyFromCurrentPage(): Promise<ApplyRunResult> {
    ...
    runsRepo.updateStatus(runId, resultStatus, { currentUrl: page.url(), summary: JSON.stringify(summaryPayload) });
    return { runId, status: resultStatus, stepCount: stepCounter, outputDir };
  }
  ...
  try {
    ...
    runsRepo.updateStatus(runId, "failed", {
      error: errorMsg,
      currentUrl: page?.url() || url,
      summary: JSON.stringify(buildSummary(0)),
    });
