...
type ColumnExpectation = {
  column: string;
  type: string; // pg data_type, lowercased, e.g. "text", "integer", "bigint"
  nullable: boolean;
  default?: string; // normalized textual representation (case/quote insensitive)
};

type TableManifest = {
  table: string;
  columns: ColumnExpectation[];
  uniqueIndexes?: string[]; // index names that should be UNIQUE
  foreignKeys?: {
    constraint: string;
    column: string;
    refTable: string;
...
// Derived from the baseline migration + entities. This is the contract the
// NestJS TypeORM replacement expects the target database to satisfy.
...
function normalizeDefault(d: string | null): string | undefined {
  if (d == null) return undefined;
  return d.replace(/'/g, "").replace(/\s+/g, " ").trim().toLowerCase();
}

function columnTypeMatches(actual: string, expected: string): boolean {
  const a = actual.toLowerCase();
  const e = expected.toLowerCase();
  if (a === e) return true;
  // tolerate serial/integer equivalence
  if ((a === "integer" || a === "bigint") && (e === "integer" || e === "bigint")) return true;
  return false;
}

async function introspect(qr: any): Promise<{ actualTables: Set<string>; problems: string[] }> {
  ...
  for (const t of tables) actualTables.add(t.table_name);
  ...
  for (const expect of MANIFEST) {
    ...
    for (const ec of expect.columns) {
      ...
      if (!columnTypeMatches(ac.data_type, ec.type)) {
        problems.push(
          `TYPE MISMATCH: ${expect.table}.${ec.column} expected ${ec.type} but found ${ac.data_type}`,
        );
      }
      const isNullable = ac.is_nullable === "YES";
      if (isNullable !== ec.nullable) {
        problems.push(
          `NULLABILITY MISMATCH: ${expect.table}.${ec.column} expected nullable=${ec.nullable} but found ${isNullable}`,
        );
      }
      if (ec.default !== undefined) {
        ...
        if (actualDefault !== ec.default) {
          problems.push(
            `DEFAULT MISMATCH: ${expect.table}.${ec.column} expected default "${ec.default}" but found "${actualDefault}"`,
          );
        }
      }
    }
    ...
    for (const uq of expect.uniqueIndexes ?? []) {
      const idx = (await qr.query(
        `SELECT indexname, indisunique FROM pg_indexes i
         ...
         WHERE i.schemaname = 'public' AND i.tablename = $1 AND i.indexname = $2`,
        [expect.table, uq],
      )) as Array<{ indexname: string; indisunique: boolean }>;
      if (idx.length === 0) {
    ...
    for (const fk of expect.foreignKeys ?? []) {
      const fks = (await qr.query(
        `SELECT tc.constraint_name, kcu.column_name, ccu.table_name AS ref_table, ccu.column_name AS ref_column,
         ...
         WHERE tc.constraint_type = 'FOREIGN KEY' AND tc.table_schema = 'public' AND tc.table_name = $1 AND tc.constraint_name = $2`,
        [expect.table, fk.constraint],
      )) as Array<{
      ...
      if (f.ref_table !== fk.refTable || f.ref_column !== fk.refColumn) {
        problems.push(
          `FK TARGET MISMATCH: ${expect.table}.${fk.constraint} -> ${f.ref_table}.${f.ref_column} expected ${fk.refTable}.${fk.refColumn}`,
        );
      }
      if (fk.onDelete && f.delete_rule.toUpperCase() !== fk.onDelete) {
        problems.push(
          `FK ON DELETE MISMATCH: ${expect.table}.${fk.constraint} expected ${fk.onDelete} found ${f.delete_rule}`,
        );
      }
      if (fk.onUpdate && f.update_rule.toUpperCase() !== fk.onUpdate) {
        problems.push(
          `FK ON UPDATE MISMATCH: ${expect.table}.${fk.constraint} expected ${fk.onUpdate} found ${f.update_rule}`,
        );
      }
    }
  }
...
async function main() {
  ...
  try {
    await qr.query(
      `CREATE TABLE IF NOT EXISTS "migrations" ("id" SERIAL NOT NULL, "timestamp" BIGINT NOT NULL, "name" varchar NOT NULL, CONSTRAINT "PK_migrations" PRIMARY KEY ("id"))`,
    );
    ...
    )) as Array<{ timestamp: number; name: string }>;
