...
export async function getJobDetail(pg: JobDetailSql, jobId: string): Promise<JobDetailReadModel | null> {
  const jobRows = (await pg.query(
    `SELECT j.id, c.slug AS "companySlug", c.name AS "companyName", c.ats, j.external_id AS "externalId", j.title, j.location, j.url, j.description, j.status, j.created_at AS "createdAt", j.updated_at AS "updatedAt"
  ...
  const filterRows = (await pg.query(
    `SELECT id, verdict, score, reasons, must_have_hits AS "mustHaveHits", missing_items AS "missingItems", model, prompt_version AS "promptVersion", created_at AS "createdAt"
     FROM job_filters WHERE job_id = $1 ORDER BY created_at DESC, id ASC`,
    [jobId],
  )) as Array<Omit<FilterReadModel, "reasons" | "mustHaveHits" | "missingItems"> & { reasons: string; mustHaveHits: string; missingItems: string }>;

  const documentRows = (await pg.query(
    `SELECT id, type, status, content, file_path AS "filePath", created_at AS "createdAt" FROM job_documents WHERE job_id = $1 ORDER BY created_at DESC, id ASC`,
    [jobId],
  )) as DocumentReadModel[];

  const applicationRows = (await pg.query(
    `SELECT id, status, score, documents, notes, created_at AS "createdAt", updated_at AS "updatedAt" FROM applications WHERE job_id = $1 LIMIT 1`,
    [jobId],
