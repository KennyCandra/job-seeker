...
exports.searchJobsList = searchJobsList;
async function searchJobsList(pg, input) {
    ...
    if (input.companyName)
        where.push(`c.name = ${addParam(input.companyName)}`);
    if (input.status)
        where.push(`j.status = ${addParam(input.status)}`);
    ...
    if (minScore > 0)
        where.push(`lf.score >= ${addParam(minScore)}`);
    ...
    const buildBaseSql = (whereSql, includeFlags = false) => `
    FROM jobs j
  ...
  `, summaryParams));
    const companiesRows = (await pg.query(`SELECT DISTINCT c.name AS name FROM jobs j INNER JOIN companies c ON c.id = j.company_id ORDER BY c.name ASC`));
    const statusRows = (await pg.query(`SELECT DISTINCT status FROM jobs ORDER BY status ASC`));
    return {
        items: rows.map((row) => ({
            id: row.id,
...
//# sourceMappingURL=jobs-list.query.js.map