...
async function runApply(options) {
    ...
    const outputDir = (0, path_1.join)(paths_1.DATA_DIR, "screenshots", `apply-${jobId}-${Date.now()}`);
    ...
    async function addStep(type, label, detail = "", screenshotPath) {
        stepCounter++;
    ...
    function removeBlocker(matcher) {
        for (let i = blockers.length - 1; i >= 0; i -= 1) {
            if (matcher.test(blockers[i]))
                blockers.splice(i, 1);
        }
    }
    async function shot(p, label) {
        const path = (0, path_1.join)(outputDir, `${String(stepCounter).padStart(3, "0")}-${slug(label)}.png`);
        try {
    ...
    async function snapshotDom(p, label) {
        const fields = await p.locator("input, textarea, select, button").evaluateAll((els) => els.map((el) => {
            const input = el;
            const id = input.id || "";
        ...
        const path = (0, path_1.join)(outputDir, `${String(stepCounter).padStart(3, "0")}-${slug(label)}.json`);
        (0, fs_1.writeFileSync)(path, JSON.stringify(fields, null, 2));
    }
    function ctx() {
        return activeFrame || page;
    }
    async function handleCookieBanners(p) {
        const patterns = [
    ...
    async function findApplyButtonOrScrollToForm(p) {
        const candidates = [
    ...
    async function waitForFormFields(p, maxRetries = 8) {
    ...
    async function detectPageBlocker(p) {
    ...
    async function countApplicantFields(context) {
    ...
    async function discoverFormFields(p) {
        const result = {
            fieldsDetected: 0,
            submitFound: false,
            iframeUsed: false,
    ...
    async function fillFieldWithVerify(context, signals, value, label) {
        if (!value)
            return false;
        const fields = context.locator(FIELD_SELECTOR);
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (!signals.some((rx) => rx.test(meta)))
                continue;
            ...
            if (!filled) {
                ...
                filled = await field.evaluate((el) => el.value || "").catch(() => "");
            }
            if (!filled) {
    ...
    function hasPreferNotToDiscloseDefault() {
    ...
    function desiredChoiceForLabel(label) {
        ...
        if (/pronoun/i.test(text)) {
            ...
            if (pronouns)
                return { value: pronouns, kind: "pronouns" };
        }
        ...
        if (/gender|race|ethnic|orientation|lgbt|transgender|veteran|disability|disabled|eeo|demographic/i.test(text)) {
            if (profile.veteranStatus && /veteran/i.test(text))
                return { value: profile.veteranStatus, kind: "veteran" };
            if (profile.disabilityStatus && /disability|disabled/i.test(text))
                return { value: profile.disabilityStatus, kind: "disability" };
            if (hasPreferNotToDiscloseDefault())
                return { value: "Prefer not to disclose", kind: "demographic" };
        }
        return null;
    }
    ...
    function scoreChoice(optionText, desired) {
        const option = normalizeChoice(optionText);
        const target = normalizeChoice(desired.value);
        if (!option || /^select|choose|please select/.test(option))
            return 0;
        if (option === target)
            return 100;
        if (option.includes(target) || target.includes(option))
            return 80;
        if (desired.kind === "country" && profile.country && option.includes(normalizeChoice(profile.country)))
            return 90;
        if (desired.kind === "pronouns" && /he\s*him/.test(target) && /he\s*him/.test(option))
            return 90;
        if (desired.kind === "sponsorship") {
            if (target === "no" && /\bno\b|do not|don't|not require|without sponsorship/i.test(optionText))
                return 90;
            if (target === "yes" && /\byes\b|require|need/i.test(optionText))
                return 90;
        }
        if (desired.kind === "work-authorization") {
            if (target === "yes" && /\byes\b|authorized|eligible/i.test(optionText))
                return 90;
            if (target === "no" && /\bno\b|not authorized|not eligible/i.test(optionText))
                return 90;
        }
        if (desired.kind === "demographic") {
            if (/prefer not|do not wish|don't wish|decline|not disclose|self-identify|not answer|choose not/i.test(optionText))
                return 95;
        }
    ...
    async function getControlLabel(control) {
    ...
    async function fillNativeSelects(context) {
        ...
        for (let i = 0; i < count; i += 1) {
            ...
            if (!desired)
                continue;
            const options = await select.locator("option").evaluateAll((els) => els.map((el) => ({
                value: el.value,
    ...
    async function fillCustomDropdowns(context) {
    ...
    async function fillDropdownChoices(context) {
        ...
        if (total > 0)
            await shot(page, "filled-dropdowns");
        return total;
    }
    async function collectFreeTextQuestionFields(context) {
        const fields = context.locator(FIELD_SELECTOR);
        const count = await fields.count();
        const questions = [];
        for (let i = 0; i < count; i += 1) {
            ...
            if (meta.value.trim())
                continue;
            if (meta.readOnly || /combobox|listbox|menu/i.test(`${meta.role} ${meta.ariaHasPopup} ${meta.ariaExpanded}`))
                continue;
            if (meta.type && !["text", "textarea", "url", "search"].includes(meta.type.toLowerCase()))
                continue;
            if (/first\s*name|last\s*name|given\s*name|family\s*name|surname|email|phone|mobile|telephone|linkedin|website|portfolio|github|city|country|location|resume|cv|cover\s*letter/i.test(haystack)) {
                continue;
            }
            questions.push({
                index: i,
    ...
    async function answerFreeTextQuestions(p, jobContext) {
        const context = ctx();
        const questions = await collectFreeTextQuestionFields(context);
        (0, fs_1.writeFileSync)((0, path_1.join)(outputDir, "ai-question-fields.json"), JSON.stringify(questions, null, 2));
        if (!questions.length)
            return;
        ...
        try {
            ...
            const parsed = (0, llm_1.parseJsonFromText)(raw);
            answers = Array.isArray(parsed.answers) ? parsed.answers : [];
        }
        catch (error) {
            (0, fs_1.writeFileSync)((0, path_1.join)(outputDir, "ai-answer-error.json"), JSON.stringify({ error: error.message, fields: questions }, null, 2));
            await addStep("error", "ai-answer-failed", error.message);
            return;
        }
        (0, fs_1.writeFileSync)((0, path_1.join)(outputDir, "ai-answers.json"), JSON.stringify(answers, null, 2));
        for (const answer of answers) {
            ...
            if (answer.needsUser) {
                ...
                if (!blockers.includes(question.label))
                    blockers.push(`Needs user input: ${question.label}`);
    ...
    async function collectFileInputMeta(context) {
        ...
        for (let i = 0; i < count; i += 1) {
            ...
            const meta = await input.evaluate((el, index) => {
                ...
                for (let depth = 0; parent && depth < 6; depth += 1) {
                    ...
                    if (text)
                        ancestorText.push(text.slice(0, 700));
    ...
    function scoreFileInput(meta, signals, blockers) {
        const idName = `${meta.id} ${meta.name}`;
        const labelText = `${meta.labels.join(" ")} ${meta.aria}`;
        const allText = `${idName} ${labelText} ${meta.nearbyText}`;
        if (blockers.some((rx) => rx.test(allText)))
            return 0;
        let score = 0;
        for (const signal of signals) {
            if (signal.test(idName))
                score += 6;
            if (signal.test(labelText))
                score += 4;
            if (signal.test(meta.nearbyText))
                score += 2;
        }
        return score;
    }
    async function uploadFileBySignals(context, kind, filePath, signals, blockerSignals) {
        if (!filePath) {
    ...
    function buildSummary(requiredEmptyCount) {
        return {
    ...
    async function pauseForHumanVerification(pageBlocker) {
        if (!blockers.includes(pageBlocker))
        ...
        if (keepBrowserOnBlock) {
            ...
            (0, sessions_1.registerPausedApplySession)({
                runId,
                browser,
                page,
    ...
    async function resumeAfterHumanVerification() {
        ...
        (0, sessions_1.clearPausedApplySession)(runId);
        try {
    ...
    async function continueApplyFromCurrentPage() {
        const pageBlocker = await detectPageBlocker(page);
        if (pageBlocker) {
        ...
        if (await fillFieldWithVerify(fillContext, [/first\s*name/i, /given\s*name/i], profile.firstName, "first-name"))
            fieldsFilled++;
        if (await fillFieldWithVerify(fillContext, [/last\s*name/i, /family\s*name/i, /surname/i], profile.lastName, "last-name"))
            fieldsFilled++;
        if (await fillFieldWithVerify(fillContext, [/email/i], profile.email, "email"))
            fieldsFilled++;
        if (await fillFieldWithVerify(fillContext, [/phone|mobile|telephone/i], profile.phone, "phone"))
            fieldsFilled++;
        if (await fillFieldWithVerify(fillContext, [/linkedin/i], profile.linkedin, "linkedin"))
            fieldsFilled++;
        if (await fillFieldWithVerify(fillContext, [/website|portfolio|github|personal\s*site/i], profile.website, "website"))
            fieldsFilled++;
        if (await fillFieldWithVerify(fillContext, [/city|location/i], profile.city, "city"))
            fieldsFilled++;
        if (await fillFieldWithVerify(fillContext, [/country/i], profile.country, "country"))
            fieldsFilled++;
        ...
        const requiredEmpty = await requiredEmptyContext.locator("input[required], textarea[required], select[required]").evaluateAll((els) => els
            .map((el) => {
            const input = el;
            return {
                tag: input.tagName.toLowerCase(),
        ...
        (0, fs_1.writeFileSync)((0, path_1.join)(outputDir, "required-empty.json"), JSON.stringify(requiredEmpty, null, 2));
